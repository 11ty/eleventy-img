...
test("Sync with two widths", t => {
	...
	t.is(stats.webp[0].width, 500);
	t.is(stats.webp[1].width, 300);
	t.is(stats.jpeg.length, 2);
	t.is(stats.jpeg[0].width, 500);
	t.is(stats.jpeg[1].width, 300);
});
...
test("Sync with null width", t => {
	...
	t.is(stats.webp.length, 2);
	t.is(stats.webp[0].width, 1280);
	t.is(stats.webp[0].height, 853);
	t.is(stats.webp[1].width, 300);
	t.is(stats.webp[1].height, 199);
	t.is(stats.jpeg.length, 2);
	t.is(stats.jpeg[0].width, 1280);
	t.is(stats.jpeg[0].height, 853);
	t.is(stats.jpeg[1].width, 300);
	t.is(stats.jpeg[1].height, 199);
});